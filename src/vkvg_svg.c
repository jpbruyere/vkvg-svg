...
uint32_t         _get_element_hash(void *elt) { return ((svg_element_header *)elt)->hash; }
svg_element_type _get_element_type(void *elt) { return ((svg_element_header *)elt)->type; }
...
void _parse_path_d_attribute(svg_context *svg, char *str) {
    ...
    while (!STR_EOF(tmp)) {
        ...
        case 'A': // rx ry x-axis-rotation large-arc-flag sweep-flag x y
            ...
            if (result) {
                if (!_try_parse_flags(tmp, 2, &large, &sweep) || !_try_parse_floats(tmp, 2, &x, &y))
                    return;
...
void _draw_text(svg_context *svg, FILE *f, svg_attributes *attribs, svg_length_or_percentage *x,
                svg_length_or_percentage *y, svg_length_or_percentage *dx, svg_length_or_percentage *dy) {
    if (attribs->hasFill) {
        if (attribs->hasFill == svg_paint_type_pattern)
            set_pattern(svg, attribs->fill);
        else
...
int skip_children(svg_context *svg, FILE *f, svg_attributes attribs, void *parentData) {
    int res = 0;
    while (!feof(f)) {
        read_element_start skip_element
    }
    return res;
}
...
int skip_attributes_and_children(svg_context *svg, FILE *f, svg_attributes attribs) {
    ...
    read_tag_end if (res > 0) res = skip_children(svg, f, attribs, NULL);
    return res;
}
